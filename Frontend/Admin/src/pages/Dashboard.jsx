...
export default function Dashboard() {
  ...
  const stats = [
    {
      title: 'Online Now',
      value: onlineCount ?? '—',
      change: 'Live', icon: Users, color: 'indigo', trend: 'up'
    },
    {
      title: 'Active Exams',
      value: data.sessions.filter(s => s.status === 'ongoing').length,
      change: 'Current', icon: Clock, color: 'sky', trend: 'up'
    },
    {
      title: 'Total Violations',
      value: totalViolations,
      change: totalViolations > 10 ? 'Alert' : 'Stable',
      icon: AlertTriangle, color: 'red', trend: totalViolations > 10 ? 'up' : 'down'
    },
    {
      title: 'Avg Integrity',
      value: `${avgIntegrity}%`,
      change: 'Excellent', icon: Award, color: 'emerald', trend: 'up'
    },
  ];
  ...
  return (
    <div className="min-h-full pb-12">
      ...
      <div className="flex items-end justify-between mb-12">
        ...
        >
          <div className="bg-white px-5 py-3 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-3">
            <div className={`w-3 h-3 rounded-full ${loading ? 'bg-amber-400 animate-pulse' : 'bg-emerald-500 animate-pulse'}`} />
            <span className="font-medium text-slate-700 font-mono text-sm tracking-widest">
              {loading ? 'LOADING...' : 'LIVE MONITOR'}
            </span>
          </div>
        </motion.div>
      </div>
      ...
      {/* ── Error banner ─────────────────────────────────────────── */}
      {error && (
        <div className="mb-8 p-4 bg-red-50 border border-red-200 rounded-2xl text-red-600 font-medium text-sm">
          ⚠️ {error}
        </div>
      )}

      {/* ── Stat cards: show skeleton OR real data ──────────────── */}
            ...
            >
              <div className={`bg-white rounded-3xl p-8 shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group ${stat.color === 'red' && totalViolations > 5 ? 'border-red-200 bg-red-50/20' : ''}`}>
                <div className="flex justify-between items-start">
                  ...
                  <div className={`px-4 py-1.5 rounded-2xl text-[10px] uppercase font-bold tracking-widest
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-3xl font-bold">
                    {loading && !cached ? '—' : totalViolations > 5 ? 'Elevated' : 'Safe'}
                  </p>
                </div>
              </div>
              <div className="text-7xl font-bold tracking-tighter mb-2">
                {loading && !cached ? '—' : avgIntegrity}
              </div>
        ...
        <div className="lg:col-span-12">
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
            ...
            <div className="space-y-4">
              ...
              {loading && !cached
                ? Array(3).fill(0).map((_, i) => <ViolationRowSkeleton key={i} />)
                      ...
                      <div className="flex-1 px-12">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-slate-600 font-bold text-xs uppercase tracking-widest">
                            {session.violationsCount} Violation Events
                          </p>
                        </div>
                      </div>
